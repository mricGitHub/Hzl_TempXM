...
//        [self.player replaceCurrentItemWithPlayerItem:[AVPlayerItem playerItemWithAsset:[[AVAsset assetWithURL:[]]]
        ...
        [self.playButton setImage:[[UIImage imageNamed:@"pl_play.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
        [self.playButton setImage:[[UIImage imageNamed:@"pl_stop.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateSelected];
        [self.playButton addTarget:self action:@selector(plPlayButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:self.playButton];
        
        
        //开始的时间label(触摸显示)
        self.beginTimeLabel = [self plCreateLabelWithTextAlignment:NSTextAlignmentRight textColor:[UIColor colorWithRed:17 / 255.0 green:205 / 255.0 blue:110 / 255.0 alpha:1] font:13 numberOfLines:0 lineBreakMode:NSLineBreakByWordWrapping];
        [self addSubview:self.beginTimeLabel];
        
        //结束时间label(触摸显示)
        self.endTimeLabel = [self plCreateLabelWithTextAlignment:NSTextAlignmentLeft textColor:[UIColor colorWithRed:17 / 255.0 green:205 / 255.0 blue:110 / 255.0 alpha:1] font:13 numberOfLines:0 lineBreakMode:NSLineBreakByWordWrapping];
        ...
        [self.slider setThumbImage:[UIImage imageNamed:@"圆.png"] forState:UIControlStateNormal];
        //添加滑动时间
...
#pragma mark ----masary适配----
/**
 适配
 */
- (void)plAddMasonryForUI {
    
    __weak typeof(self) weakSelf = self;
    
    //当前view的宽
    CGFloat currentViewWidth = self.bounds.size.width;
    //当前加载的video的宽
    CGFloat videoWidth = self.playerLayer.videoRect.size.width;
    
    //label的实际起始位置
    CGFloat startLocation = (currentViewWidth - videoWidth) / 2;
    ...
    [self.beginTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //左
        make.left.mas_equalTo(weakSelf).offset(startLocation);
        
        //右
        make.right.mas_equalTo(weakSelf.slider.mas_left).offset(-10.0f);
    ...
    [self.endTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //左
        make.left.mas_equalTo(weakSelf.slider.mas_right).offset(10.0f);
        
        //右
        make.right.mas_equalTo(weakSelf).offset(-startLocation);
    ...
    [self.progressView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //左
        make.left.mas_equalTo(weakSelf.beginTimeLabel.mas_right).offset(10);
        
        //右
        make.right.mas_equalTo(weakSelf.endTimeLabel.mas_left).offset(-10);
    ...
    [self.horizontalLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        //上
        make.top.mas_equalTo(weakSelf.playButton.mas_bottom);
...
- (void)plMoviePlayDidEnd {
    ...
    //调用释放函数
    if (self.releaseCurrentPageBlock != nil) {
